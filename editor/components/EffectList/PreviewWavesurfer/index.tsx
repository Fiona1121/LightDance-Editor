...
const Wavesurfer = ({ cleanMode = false }) => {
    const { waveSurferApp, initWaveSurferApp, showMarkers } = useContext(WaveSurferAppContext) as wavesurferContext;
    ...
    // update Pos Markers
    useEffect(() => {
        if (editor === POS_EDITOR) if (!posLoading && posMap && showMarkers) waveSurferApp.updateMarkers(posMap);
    }, [editor, posRecord, posMap]);

    // update Markers when markers switched on
    useEffect(() => {
        if (controlLoading || posLoading || !controlMap || !posMap || !waveSurferApp) return;

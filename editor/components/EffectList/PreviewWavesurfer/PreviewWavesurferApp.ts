...
import MarkersPlugin from "components/Wavesurfer/MarkersPlugin";
...
import { setCurrentPreviewTime, setIsPreviewing } from "core/actions";
...
class PreviewWaveSurferApp {
    ...
    playLoop() {
        ...
        const setRegion = (r: Region) => {
            if (this.waveSurfer.getCurrentTime() <= (r.end || 0) && this.waveSurfer.getCurrentTime() >= (r.start || 0))
                Region = r;
        };
     ...
     */
    addClickEvent() {
        document.getElementById("waveform")?.addEventListener("click", (e: MouseEvent) => {
    ...
    zoom(newValue: number) {
        this.waveSurfer.zoom((newValue * (window.screen.availWidth - this.waveSurfer.params.minPxPerSec)) / 50);
    }
...
export default PreviewWaveSurferApp;

export const previewWaveSurferAppInstance = new PreviewWaveSurferApp();

...
class Controller {
    ...
    init(effectName: string) {
        ...
        this.pixiApp = new PIXI.Application({
            resizeTo: document.getElementById("preview")!,
            backgroundColor: 0x000000,
        });
        this.mainContainer = new PIXI.Container();
        this.mainContainer.sortableChildren = true;
        this.pixiApp.stage.addChild(this.mainContainer);
        document.getElementById("preview_stage")!.appendChild(this.pixiApp.view);

        // initialization for dancers
        const { dancerNames } = store.getState().load;
        dancerNames.forEach((name: string, idx: number) => {
            this.dancers[idx] = new Dancer(idx, idx, this.pixiApp, store.getState().load.texture, this.mainContainer);
        });

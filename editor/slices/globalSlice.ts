...
// utils
import { clamp, updateFrameByTime, updateFrameByTimeMap, interpolationPos, fadeStatus } from "../utils/math";
...
export const globalSlice = createSlice({
    ...
    reducers: {
        ...
        controlInit: (
            ...
            if (controlRecord.length === 0) throw new Error(`[Error] controlInit, controlRecord is empty `);
        ...
        posInit: (state, action: PayloadAction<posRecordType>) => {
            ...
            if (posRecord.length === 0) throw new Error(`[Error] posInit, posRecord is empty `);
        ...
        setCurrentFade: (state, action: PayloadAction<boolean>) => {
            ...
            if (typeof fade !== "boolean") throw new Error(`[Error] setCurrentFade, invalid paramter(fade), ${fade}`);
        ...
        saveCurrentFade: (state) => {
            state.controlMap[state.controlRecord[state.timeData.controlFrame]].fade = state.currentFade;
        ...
        editCurrentStatusLED: (
            ...
            if (src && src !== "") (state.currentStatus[dancerName][partName] as LED).src = src;
        ...
        setCurrentPosByName: (
            ...
            if (!state.currentPos[name]) throw new Error(`[Error] setCurrentPos, invalid parameter(name), ${name}`);
        ...
        setTime: (state, action: PayloadAction<{ from: string; time: number }>) => {
            ...
            if (newControlFrame === state.controlRecord.length - 1) {
                // Can't fade
                state.currentStatus = state.controlMap[state.controlRecord[newControlFrame]].status;
            } else {
                // do fade
            ...
            const newPosFrame = updateFrameByTime(state.posRecord, state.timeData.posFrame, time);
            ...
            // set currentFade
            state.currentFade = state.controlMap[state.controlRecord[newControlFrame]].fade;
        },
        ...
        setControlFrame: (
            ...
            state.timeData.time = state.controlMap[state.controlRecord[controlFrame]].start;
            state.currentStatus = state.controlMap[state.controlRecord[controlFrame]].status;
            ...
            state.currentPos = state.posRecord[newPosFrame].pos;
            // set currentFade
            state.currentFade = state.controlMap[state.controlRecord[controlFrame]].fade;
        },
        ...
        toggleMode: (state, action: PayloadAction<number>) => {
            if (action.payload === state.mode) {
                ...
                if (currentControlFrame === state.controlRecord.length - 1) {
                    // Can't fade
                    state.currentStatus = state.controlMap[state.controlRecord[currentControlFrame]].status;
                } else {
                    // do fade
                    state.currentStatus = fadeStatus(
                        state.timeData.time,
         ...
         */
        editLightPresetsName: (state, action: PayloadAction<{ name: string; idx: number }>) => {
